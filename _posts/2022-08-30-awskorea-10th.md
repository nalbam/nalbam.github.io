...
국내에서 열리는 AWS DeepRacer 경기를 참여하면서 랩타임이 사람손에 의해서 정확하지 않다라는 생각을 하게 되었습니다. 그래서 `압력감지 센서` 와 `라즈베리파이` 를 이용해서 [딥레이서 타이머](https://nalbam.github.io/2019/11/07/deepracer-timer-ko.html) 를 제작하였습니다. 많은 분들이 좋아해 주셨고, AWS 에서도 이걸 참고해서 만들고 계시던데.. 이후 소식이 궁금 하네요. ㅎㅎ
